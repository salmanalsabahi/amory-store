    ...
    return (
      <div className="pt-32 pb-20 bg-slate-50 min-h-screen flex items-center justify-center px-4 text-center">
        <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-100 max-w-lg mx-auto">
          <div className="w-24 h-24 bg-amber-50 rounded-full flex items-center justify-center mb-8 text-amber-600 mx-auto">
          ...
          <button 
            ...
            className="w-full bg-slate-900 text-white px-10 py-4 rounded-2xl font-black text-lg hover:bg-amber-600 hover:text-slate-900 transition-all active:scale-95 shadow-xl shadow-slate-900/10"
          >
            تحديث الصفحة
          </button>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-20">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-500/10 text-amber-600 font-black text-sm mb-6 border border-amber-500/20"
          >
          ...
          >
            عروض لا تُقاوم <span className="text-amber-500">لأناقتك</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        {offers.length === 0 && !loading && !message && (
          ...
          >
            ...
            <Link to="/store" className="inline-flex items-center gap-2 mt-8 text-amber-600 font-black hover:translate-x-[-4px] transition-all">
            ...
            >
              ...
              <div className="relative w-full md:w-2/5 min-h-[160px] sm:min-h-[220px] md:min-h-[300px] bg-slate-50 overflow-hidden">
                ...
                <div className="absolute top-3 right-3 md:top-6 md:right-6">
                  <div className="bg-amber-500 text-slate-950 font-black px-2 py-1 text-[10px] md:text-base md:px-4 md:py-2 rounded-xl md:rounded-2xl shadow-xl border border-white/20 transform -rotate-3 group-hover:rotate-0 transition-transform">
                    -{offer.discount}% خصم
                  </div>
                </div>
              ...
              <div className="p-4 sm:p-6 md:p-10 flex-1 flex flex-col">
                <div className="flex items-center gap-1 md:gap-2 text-amber-600 font-bold text-[10px] md:text-xs uppercase tracking-widest mb-2 md:mb-4">
                  <Clock className="w-3 h-3 md:w-3.5 md:h-3.5" /> <span className="truncate">عرض لفترة محدودة</span>
                </div>
                <h3 className="text-base sm:text-xl md:text-3xl font-display font-black text-slate-900 mb-2 md:mb-4 tracking-tight group-hover:text-amber-600 transition-colors line-clamp-2 md:line-clamp-none">{offer.title}</h3>
                ...
                <div className="flex flex-col sm:flex-row gap-2 md:gap-4 mt-auto">
                  <button
                    ...
                    className="flex-1 bg-slate-900 text-white px-3 py-2 sm:px-4 sm:py-3 md:px-8 md:py-4 rounded-xl md:rounded-2xl font-black text-[13px] sm:text-sm md:text-lg hover:bg-amber-600 hover:text-slate-900 transition-all flex items-center justify-center gap-1 md:gap-3 disabled:opacity-50 active:scale-95 shadow-xl shadow-slate-900/10"
                  >
        ...
        <div className="mt-32 p-12 lg:p-20 bg-slate-950 rounded-[4rem] text-center relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-6 relative z-10">
            <Link to="/contact" className="bg-amber-500 text-slate-900 px-10 py-5 rounded-2xl font-black text-xl hover:bg-white transition-all shadow-xl shadow-amber-500/10 active:scale-95">
              تواصل معنا الآن
            </Link>
