      ...
      {loading ? (
        ...
        >
          <div className="w-24 h-24 bg-amber-50 rounded-full flex items-center justify-center mb-8">
            <PackageIcon className="w-12 h-12 text-amber-600" />
          </div>
          ...
          <div className="flex gap-4">
            <Link to="/store" className="bg-amber-600 hover:bg-amber-700 text-white px-10 py-4 rounded-2xl font-black transition-all shadow-xl shadow-amber-600/20 active:scale-95">
              تصفح المتجر الآن
            </Link>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-8">
          {packages.map((pkg, idx) => (
            ...
            >
              <div className="w-16 h-16 md:w-24 md:h-24 rounded-full bg-slate-50 text-amber-600 flex items-center justify-center mb-4 md:mb-6 group-hover:scale-110 group-hover:bg-amber-600 group-hover:text-white transition-all duration-500 shadow-sm">
                <PackageIcon className="w-8 h-8 md:w-12 md:h-12" />
              </div>
              
              <h3 className="text-[13px] sm:text-base md:text-2xl font-display font-black text-slate-900 mb-1 md:mb-3 group-hover:text-amber-600 transition-colors uppercase tracking-tight line-clamp-2 md:line-clamp-none">{pkg.title}</h3>
              <p className="text-slate-500 mb-2 md:mb-4 text-[10px] md:text-sm font-medium line-clamp-2 md:line-clamp-none">{pkg.subtitle}</p>
              <div className="text-sm sm:text-lg md:text-2xl font-display font-black text-amber-600 mb-4 md:mb-8">{pkg.price.toLocaleString()} <span className="text-[9px] md:text-xs font-normal text-slate-400">ريال</span></div>
              
              <button 
                onClick={() => handleAddToCart(pkg)}
                disabled={addingId === pkg.id}
                className="mt-auto w-full bg-slate-900 hover:bg-amber-600 text-white rounded-xl md:rounded-2xl py-2 md:py-4 font-black transition-all shadow-lg shadow-slate-900/10 active:scale-95 disabled:opacity-50 flex items-center justify-center gap-1 md:gap-2 text-[11px] sm:text-sm md:text-base"
              >
                {addingId === pkg.id ? (
                  <>
