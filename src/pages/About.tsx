  ...
  return (
    <div className="bg-white">
      ...
      <section className="relative pt-24 md:pt-32 pb-16 md:pb-24 overflow-hidden font-body">
        <div className="absolute top-0 right-0 w-full md:w-1/3 h-full bg-rose-50/50 -z-10 rounded-bl-[50px] md:rounded-bl-[100px]" />
          ...
          <div className="grid lg:grid-cols-2 gap-10 md:gap-16 items-center">
            ...
            >
              ...
              <h1 className="text-3xl sm:text-4xl md:text-6xl font-display font-black text-slate-900 leading-tight mb-4 md:mb-8">
                {settings?.storeName || "عموري للتجميل"} <br />
              ...
              <p className="border-r-2 border-r-rose-400 pr-3 md:border-none md:pr-0 text-sm md:text-lg text-slate-600 leading-relaxed max-w-xl whitespace-pre-wrap">
                {settings?.aboutUs || "نحن في عموري للتجميل، نؤمن بأن الأناقة ليست ترفاً، بل هي لغة تعبر عن الرقي والتميز. نتخصص في تقديم تشكيلة حصرية من أرقى الساعات العالمية والعطور التي تأخذك في رحلة من السحر والجاذبية."}
              </p>
            </motion.div>
            ...
            >
              <div className="absolute -top-4 -right-4 md:-top-6 md:-right-6 w-24 h-24 md:w-32 md:h-32 bg-rose-100 rounded-full blur-2xl md:blur-3xl opacity-60" />
              <div className="absolute -bottom-4 -left-4 md:-bottom-6 md:-left-6 w-32 h-32 md:w-48 md:h-48 bg-rose-100 rounded-full blur-2xl md:blur-3xl opacity-60" />
          ...
          >
            {stats.map((stat) => (
              <motion.div key={stat.id} variants={itemVariants} className="text-center group">
                <div className="inline-flex w-12 h-12 md:w-16 md:h-16 items-center justify-center rounded-xl md:rounded-2xl bg-rose-500/50 text-white mb-4 md:mb-6 group-hover:bg-white group-hover:text-rose-600 transition-all duration-300">
                  <stat.icon className="w-6 h-6 md:w-8 md:h-8" />
                </div>
                <div className="text-2xl sm:text-3xl md:text-4xl font-display font-black text-white mb-1 md:mb-2">{stat.value}</div>
                <div className="text-rose-100 text-[10px] md:text-sm font-medium">{stat.label}</div>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      <section className="py-16 md:py-24 bg-slate-50 relative overflow-hidden font-body">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-6 md:gap-12">
            ...
            >
              <div className="w-12 h-12 md:w-14 md:h-14 bg-rose-50 rounded-xl md:rounded-2xl flex items-center justify-center mb-6 md:mb-8">
                <Target className="w-6 h-6 md:w-8 h-8 text-rose-600" />
              </div>
      ...
      <section className="py-16 md:py-24 font-body">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-10 md:mb-20">
            <h2 className="text-2xl md:text-4xl font-display font-bold text-slate-900 mb-4 md:mb-6 underline decoration-rose-100 decoration-8 underline-offset-4 pointer-events-none">قيمنا الجوهرية</h2>
          ...
          >
            {values.map((value) => (
              <motion.div 
                ...
                className="p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] border border-slate-100 hover:border-rose-100 transition-all hover:bg-rose-50/10 group"
              >
      ...
      <section className="py-16 md:py-24 bg-slate-950 overflow-hidden relative font-body">
        <div className="absolute top-0 left-0 w-full h-full opacity-10 pointer-events-none">
          ...
          <div className="absolute bottom-0 right-0 w-64 h-64 md:w-96 md:h-96 bg-rose-400 rounded-full blur-[100px] md:blur-[120px] translate-x-1/2 translate-y-1/2" />
        </div>
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <p className="text-rose-100/80 text-sm md:text-lg mb-8 md:mb-12 leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row gap-4 md:gap-6 justify-center">
            <motion.a 
              ...
              whileTap={{ scale: 0.95 }}
              className="px-8 py-3 md:px-10 md:py-4 text-sm md:text-base bg-rose-500 text-slate-950 rounded-xl md:rounded-2xl font-bold shadow-xl shadow-rose-500/20"
            >
              تواصل معنا الآن
            </motion.a>
            <motion.a 
